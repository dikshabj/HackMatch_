...
const ForgotPassword = () => {
  ...
  const handleSendOtp = async (e) => {
    ...
    try {
      await axios.post(`http://localhost:8083/api/v1/auth/forgot-password?email=${email}`);
  ...
  const handleVerifyOtp = async (e) => {
    ...
    setMessage({ type: '', text: '' });
    try {
      await axios.post(`http://localhost:8083/api/v1/auth/verify-otp?email=${email}&otp=${otp}`);
  ...
  const handleResetPassword = async (e) => {
    ...
    setMessage({ type: '', text: '' });
    try {
      await axios.post(`http://localhost:8083/api/v1/auth/reset-password?email=${email}&otp=${otp}&newPassword=${newPassword}`);
